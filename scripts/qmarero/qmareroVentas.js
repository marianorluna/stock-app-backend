 ...
 *
 * Requiere en .env: Q_API_URL, Q_API_BODY, MONGODB_URI
 * El bearer se obtiene de la base de datos (colección Config, campo value_qm_bearer)
 * Uso: 
...
/**
 * Obtiene el bearer token de la base de datos
 */
async function obtenerBearerDeBD() {
    const config = await Config.findOne();

    if (!config || !config.value_qm_bearer) {
        throw new Error('No se encontró value_qm_bearer en la base de datos. Por favor, configura el bearer desde la interfaz de Configuraciones.');
    }

    return config.value_qm_bearer;
}

/**
 * Llama a la API de Qamarero.
 * El bearer se obtiene de la base de datos (colección Config, campo value_qm_bearer).
 */
async function llamarQamarero(inputExtra = {}, bearer) {
    ...
    if (!bearer && !cookie) throw new Error('Falta bearer en la base de datos o Q_COOKIE en variables de entorno');
 ...
 */
async function obtenerTicketsDeRango(fechaInicio, fechaFin, bearer) {
    const tickets = [];
    let page = 1;
    const MAXP = 60;
...
async function main() {
    ...
    // Obtener el bearer de la base de datos
    let bearer;
    try {
        bearer = await obtenerBearerDeBD();
        console.log('✅ Bearer obtenido de la base de datos');
    } catch (error) {
        console.error('❌ Error obteniendo bearer:', error.message);
    ...
    let output;
    try {
        const tickets = await obtenerTicketsDeRango(fechaInicio, fechaFin, bearer);
        const fechaInicioStr = formatearFecha(fechaInicio);
        const fechaFinStr = formatearFecha(fechaFin);
